...
class _TaskDetailItemState extends State<TaskDetailItem> with SingleTickerProviderStateMixin {
    ...
    return AnimatedBuilder(
      ...
      builder: (ctx, child) {
        return Transform.translate(
          offset: Offset((size.width + widget.index * 100) * (_animation.value - 1), 0),
          child: child,
        );
      },
      child: Container(
            ...
            Row(
              children: <Widget>[
                ...
                Expanded(
                  ...
                  child: IconButton(
                    icon: Icon(
                      progressShow ? Icons.arrow_drop_up : Icons.arrow_drop_down,
                      color: Colors.grey,
                    ),
                    onPressed: () {
                      setState(() {
  ...
  void dispose() {
    timer?.cancel();
    _controller?.dispose();
    debugPrint("taskDetailItem ${widget.index} disposed");
    super.dispose();
  }
}
